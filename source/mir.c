...
//if(oip[x] == 0 || oip[x] == 150) // XXX kludge for full placements.pgm AWW
		if(V == skipval) // skip black input - for out of bounds
			continue;
if(backp) backp[iy*iwid+ix] = oip[x]; // copy from prev output to backp
		oip[x] = lut[V+128];
	}
...
if(ibpp != 1 || obpp != 1) { fprintf(stderr, "EXIT\n"); exit(1); }
	for(x = x0; x <= x1; x++, tx += mf[0][0], ty += mf[1][0]) {
	...
	for(x = x0; x <= x1; x++, tx += mf[0][0], ty += mf[1][0]) {
		...
		f3 = p[1] + 0.5 * frx*(p[2] - p[0] + frx*(2.0*p[0] -
			5.0*p[1] + 4.0*p[2] - p[3] +
				frx*(3.0*(p[1] - p[2]) + p[3] - p[0])));
		V = f1 + 0.5 * fry*(f2 - f0 + fry*(2.0*f0 -
			5.0*f1 + 4.0*f2 - f3 + fry*(3.0*(f1 - f2) + f3 - f0)));
		if(V == 0)
			continue;
		oip[x] = lut[V+128];
	}
...
*/
	...
	for(y0 = (int)(v0[1]+.5); y0 <= (int)(v1[1]+.5); y0++) {
		f0 = (v1[1] - y0) / dy0;
		f1 = (v2[1] - y0) / dy2;
if(f0 < 0) { fprintf(stderr, "reset %g to 0\n", f0); f0 = 0; }
		for(i = 0; i < VL; i++) {
...
//fprintf(stderr, "owid %d oht %d obpp %d\n", owid, oht, obpp);
					...
					obase = outimg->pp;
					if(Zval)
						memset(obase, Zval>255?255:Zval,
							obpp*(long)owid*oht);
				}
...
fprintf(stderr, "%d %d  ", x, y);
	if(x < box_xmin) box_xmin = x;
	if(y < box_ymin) box_ymin = y;
	if(x > box_xmax) box_xmax = x;
	if(y > box_ymax) box_ymax = y;
