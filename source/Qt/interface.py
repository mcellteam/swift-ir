...
"""
import sys, os, re, traceback, copy, json, psutil, inspect, threading, concurrent.futures, collections, time, random, \
    multiprocessing, logging, textwrap
...
from qtpy.QtGui import QPixmap, QColor, QPainter, QPen, QIntValidator, QDoubleValidator, QIcon, QSurfaceFormat, \
    QPaintEvent, QBrush, QFont, QTextCursor, QOpenGLContext
from qtpy.QtCore import Qt, QRectF, QSize, QPoint, QPointF, QUrl, QRunnable, QObject, QAbstractAnimation, \
    QPropertyAnimation, QParallelAnimationGroup, QProcess, QThreadPool, QThread, Signal, Slot
...
from package.em_utils import getCurScale, isDestinationSet, isProjectScaled, \
    ...
    ensure_proper_data_structure, clear_all_skips, verify_image_file,getCurLayer
...
from package.compute_affine_transformations import compute_affine_transformations
...
class MainWindow(QMainWindow):
    ...
    def run_scaling(self) -> None:
        ...
        from package import generate_scales
    ...
    def run_regenerate_alignment(self) -> None:
        ...
        else:  self.hud.post('There Was a Problem. Try Re-aligning First.', logging.ERROR);  self.set_idle()
    ...
    def make_zarr_multithreaded(self, aligned_path, n_scales, cname, clevel, destination_path, ds_name):
        os.system("python3 convert_zarr.py %s -c '64,64,64' -nS %s -cN %s -cL %s -d %s -n %s" % (aligned_path, str(n_scales), str(cname), str(clevel), destination_path, ds_name))
    ...
    def export_zarr(self):
            ...
            if self.cname == "none":
                ...
                os.system("python3 convert_zarr.py %s -c '64,64,64' -nS %s -nC 1 -d %s -n %s" % (self.aligned_path, self.n_scales, self.dest_path, self.ds_name))
                ...
                os.system("python3 convert_zarr.py %s -c '64,64,64' -nS %s -cN %s -cL %s -d %s -n %s" % (self.aligned_path, str(self.n_scales), str(self.cname), str(self.clevel), destination_path, self.ds_name))
...
class HeadsUpDisplay(QWidget):
    ...
    def __init__(self, app):
        ...
        # fs = '%(asctime)s %(qThreadName)-15s %(levelname)-8s %(message)s'
        fs = '%(asctime)s [%(levelname)s] %(qThreadName)-15s | %(message)s'
        # fs = '%(levelname)-8s %(asctime)s %(qThreadname)-15s %(message)s'
