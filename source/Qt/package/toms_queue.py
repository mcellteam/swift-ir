...
def worker(worker_id, task_q, result_q, n_tasks, n_workers, pbar_q = None):
    ...
    for task_id, task in  iter(task_q.get, 'END_TASKS'):
        ...
        # sys.stderr.write('toms_queue.py | Worker %d:  Running Task %d\n' % (worker_id, task_id))
        ...
        try:
            ...
            # sys.stderr.write('toms_queue.py | Worker %d:  Task %d Completed with RC %d\n' % (worker_id, task_id, rc))
        except:
            outs = ''
...
class TaskQueue:
    ...
    def start(self, n_workers, retries=1) -> None:
        ...
        '''task_queue_mp.TaskQueue.start | type(self.progress_callback) =  <class 'PyQt5.QtCore.pyqtBoundSignal'>
        task_queue_mp.TaskQueue.start | str(self.progress_callback) =  <bound PYQT_SIGNAL progressSignal of WorkerSignals object at 0x186f84dc0>'''
        ...
        print('task_queue_mp.start | self.n_tasks = ', self.n_tasks)
    ...
    def collect_results(self) -> None:
        ...
        while (retries_tot < self.retries + 1) and n_pending:
            ...
            print('task_queue_mp.collect_results | n_pending = %d' % n_pending)
            self.end_tasks()
            ...
            if (retries_tot < self.retries) and n_pending:
                sys.stderr.write('task_queue_mp | Requeuing Any Failed Tasks...')
                sys.stderr.write('task_queue_mp | Failed Tasks: %d' % n_pending)
                sys.stderr.write('task_queue_mp |     Task IDs: %s' % str(retry_list))

                self.restart()
                for task_id in retry_list:
                    sys.stderr.write('task_queue_mp | Requeuing Failed Task ID: %d   Retries: %d' % (task_id, retries_tot + 1))
                    # sys.stderr.write('task_queue_mp |                     Task: %s' % (str(self.task_dict[task_id])))
...
print('task_queue_mp.TaskQueue.start | type(self.progress_callback) = ', type(self.progress_callback))
print('task_queue_mp.TaskQueue.start | str(self.progress_callback) = ', str(self.progress_callback))
print('task_queue_mp.TaskQueue.start | mp.parent_process() = ', mp.parent_process())
print('task_queue_mp.TaskQueue.start | mp.get_context() = ', mp.get_context())
print('task_queue_mp.TaskQueue.start | mp.get_start_method() = ', mp.get_start_method())


image_apply_affine task (example)
task= ['/Users/joelyancey/.local/share/virtualenvs/swift-ir-AuCIf4YN/bin/python3', 
       '/Users/joelyancey/glanceem_swift/swift-ir/source/Qt/package/job_apply_affine.py', 
       '-gray', 
       '-afm', 
       '1.040024184796598', 
       '-0.06164416771039566', 
...
INFO:interface:   ARGS:   ['/Users/joelyancey/glanceem_swift/swift-ir/source/Qt/package/job_single_alignment.py', '/Users/joelyancey/glanceEM_SWiFT/test_projects/full_volume_josef_2/project_runner_job_file.json', 'init_affine', '4', 'c', '127', '1', '1']
INFO:interface:   STDERR: /Users/joelyancey/.local/share/virtualenvs/swift-ir-AuCIf4YN/bin/python3: can't open file '/Users/joelyancey/glanceem_swift/swift-ir/source/Qt/package/job_single_alignment.py': [Errno 2] No such file or directory
...
project_runner.do_alignment | Starting task_queue_mp with args:
  /Users/joelyancey/.local/share/virtualenvs/swift-ir-AuCIf4YN/bin/python3
  /Users/joelyancey/Downloads/swift-ir-joel-dev-pyside6/source/Qt/package/job_single_alignment.py
...
Starting task_queue_mp with args:
  /Users/joelyancey/.local/share/virtualenvs/swift-ir-AuCIf4YN/bin/python3
  /Users/joelyancey/glanceem_swift/swift-ir/source/Qt/package/src/job_single_alignment.py
