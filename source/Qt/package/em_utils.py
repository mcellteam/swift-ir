...
def apply_project_defaults() -> None:
    ...
    for scale_key in scales_dict.keys():
        ...
        for layer_index in range(len(scale['alignment_stack'])):
            ...
            if scale_key == coarsest_scale:
                scale['alignment_stack'][layer_index]['align_to_ref_method']['method_data'][
...
def ensure_proper_data_structure():
    ...
    for scale_key in scales_dict.keys():
        ...
        if not 'null_cafm_trends' in scale:
            scale[
                'null_cafm_trends'] = False  # 0523 need to hardcode this since cannot any longer read the value from toggle
        if not 'use_bounding_rect' in scale:
...
def print_sanity_check():
    ...
    print("  alignment_option                                 :",
          cfg.project_data['data']['scales'][getCurScale()]['method_data']['alignment_option'])
    try:
...
def isAnyScaleAligned() -> bool:
    ...
    try:
        ...
        files = ''  # 0520
        print(
            'isAnyScaleAligned | EXCEPTION | Looking for *.tif in project dir but didnt find any - Returning empty string')
...
def getNumScales() -> int:
    ...
    try:
        n_scales = len(cfg.project_data['data']['scales'])
    except:
        print('getNumScales | WARNING | Something went wrong getting the # of scales. Check project dictionary.')
