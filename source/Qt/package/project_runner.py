...
try: import package.toms_queue as task_queue
except: import toms_queue as task_queue

# try: import package.pyswift_tui as pyswift_tui
# except: import pyswift_tui

from image_funcs import SetStackCafm, BoundingRect
...
class project_runner:
    ...
    def do_alignment(self, alignment_option='init_affine', generate_images=True):
        ...
        if self.run_parallel:
            ...
            # align_job = os.path.join(my_path, 'source/Qt/package/job_single_alignment.py')
            align_job = os.path.join(my_path, 'package/job_single_alignment.py')
            print('(tag) (do_align) project_runner | do_alignment | align_job=',align_job)
            # (tag) (do_align) project_runner | do_alignment | align_job= /Users/joelyancey/glanceem_swift/swift-ir/source/Qt/package/job_single_alignment.py
            ...
            save_bias_analysis(self.project['data']['scales'][cur_scale]['alignment_stack'], bias_data_path) # <-- call to save bias data
    ...
    def generate_aligned_images(self):
        ...
        # Finally generate the images with a parallel run of job_apply_affine.py
        ...
        my_path = os.path.split(os.path.realpath(__file__))[0]
        # apply_affine_job = os.path.join(my_path, 'source/Qt/package/job_apply_affine.py')
        apply_affine_job = os.path.join(my_path, 'package/job_apply_affine.py')
        ...
        for layer in alstack[self.start_layer:end_layer + 1]:
            ...
            # print_debug ( -1, 'Run processes for: python job_apply_affine.py [ options ] -afm 1 0 0 0 1 0  in_file_name out_file_name' )
