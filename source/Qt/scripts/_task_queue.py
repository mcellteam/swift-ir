...
#    if True:
      ...
      if arg_in:
        if debug_level > 4: sys.stdout.write('run_proc in _task_queue.py with arg_in = ' + str(arg_in) + '\n')
        if debug_level > 4: sys.stdout.write('run_proc in _task_queue.py with args:\n')
        for arg in arg_in:
          char_stream = ""
          if type(arg) == type([]):
...
class TaskQueue:
  def __init__(self, python_path):
    ...
    print('(tag) task_queue | class=TaskQueue | module_file_path = ', module_file_path)
