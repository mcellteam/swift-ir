...
# THIS IS ONLY CALLED WHEN LINKING LAYERS (i.e. when importing images)
...
# THEY NEED TO BE IMPORTED FROM glancem_utils.py INSTEAD
###########################################
###########################################
###########################################
...
def view_change_callback(prev_scale_key, next_scale_key, prev_layer_num, next_layer_num, new_data_model=False):
    ...
    if alignem.project_data != None:
        ...
        if copy_from_widgets_to_data_model:
            ...
            alignem.project_data['data']['scales'][prev_scale_key]['poly_order'] = alignem.main_window.get_poly_input()
