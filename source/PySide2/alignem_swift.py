...
def generate_scales ():
    ...
    if (main_win.destination_path == None) or (len(main_win.destination_path) <= 0):
      ...
      for scale in alignem.global_image_scales:
        ...
        for al in alignem.scale_dict[alignem.current_scale]:
          try:
...
def align_all():
    ...
    if (main_win.destination_path == None) or len(main_win.destination_path) <= 0:
      ...
      for index in range(len(alignem.scale_dict[alignem.current_scale])):
        print ( "Aligning layer " + str(index) )
        al = alignem.scale_dict[alignem.current_scale][index]
