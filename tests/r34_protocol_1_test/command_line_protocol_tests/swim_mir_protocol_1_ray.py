...
@ray.remote
def render(multiargs_slice):
    
    return [run_mir(*args) for args in multiargs_slice]
...
def par(l, c):
    ...
    n, r = divmod(l, c)
    res = [(c*i, c*(i+1)) for i in range(n)]
    if r == 0:
        return res
    else:
        res.append((n * c, n * c + r))
        return res


@ray.remote
def _protocol_1(multiargs_slice):

    return [protocol_1(*args) for args in multiargs_slice]
...
def run_protocol_1(img_dir, res_dir, iter, w=-0.65, f=None,
    ...
    for scale_idx, dir in enumerate(_dirs):
        ...
        if save_signals or save_render:
            ...
            if os.path.isdir(res_scale_dir):
                pass
            else:
                os.mkdir(res_scale_dir)
        ...
        _par =  par(len(multiargs), chunksize)
        __res = []
        [__res.append(_protocol_1.remote(multiargs[i:j])) for i, j in _par]
        _res = ray.get(__res)
        res = []
        [res.extend(x) for x in _res]
        ...
        if save_render:
            ...
            _res = [render.remote(ren_args[i:j]) for i, j in _par]
            ray.get(_res)
...
if __name__ == '__main__':
    ...
    t = perf_counter()
    ray.init(num_cpus=n_proc)    # , include_dashboard=False)
    print(f'ray init time: {perf_counter() - t:.2f} sec')
