...
def remove_aligned(use_scale, project_dict, image_library, start_layer=0):
  '''
  ...
  for layer in project_dict['data']['scales'][use_scale]['alignment_stack'][start_layer:]:
    ifn = layer['images'].get('filename', None)
    layer['align_to_ref_method']['method_results'] = {}
    layer['images'].pop('aligned', None)
    if ifn != None:
      try:     os.remove(ifn)
      except:  logger.warning("os.remove(%s) Triggered An Exception" % ifn)
      
      try:     image_library.remove_image_reference(ifn)
      except:  logger.warning("image_library.remove_image_reference(%s) Triggered An Exception" % ifn)
  logger.info('<<<< image_utils.remove_aligned')
#
#
